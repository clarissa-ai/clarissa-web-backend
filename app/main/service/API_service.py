...
def post_user_response_to_api(user_json, user_sex, user_age):
    ...
    """
    #use user_id to get user's sex, age and store that in variables user_sex and user_age, input that into diagnosis endpoint
    ...
    for i in range(0, len(sym_json['mentions'])):
        data_syms.append({'id':sym_json['mentions'][i]['id'], 'choice_id': 'present'})
